...
const cancellationReasons = /* @__PURE__ */ new Map();
...
function decorateSnapshot(snapshot) {
	...
	if (reason === void 0) return snapshot;
	const detail = snapshot.detail === void 0 ? reason : `${snapshot.detail}; ${reason}`;
	return {
		...snapshot,
		detail
	};
...
function toWire(snapshot) {
	...
	return {
		...
		...visible.detail !== void 0 ? { detail: visible.detail } : {},
		startedAt: visible.startedAt,
		...visible.finishedAt !== void 0 ? { finishedAt: visible.finishedAt } : {},
		...visible.ownerSession !== void 0 ? { ownerSession: visible.ownerSession } : {}
	};
}
...
function killTask(ctx, id, sessionId) {
	...
	return {
		outcome,
		snapshot: decorateSnapshot(ctx.jobs.get(id, owned.agent))
	};
}
/**
...
function apply(ctx) {
	ctx.effect(() => {
		...
		ctx.jobs.read = (id, caller) => {
			...
			return {
				text,
				snapshot: decorateSnapshot(result.snapshot)
			};
		};
